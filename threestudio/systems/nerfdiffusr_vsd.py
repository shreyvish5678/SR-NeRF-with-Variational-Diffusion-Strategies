...
@threestudio.register("nerfdiffusr-system-vsd")
class NerfDiffuSR(BaseLift3DSystem):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.stage == 'sr':
            ...
            c2w = self.train_dataset.poses[idxs].to(self.device)
            mvp_mtx = self.dataset.mvp_mtx

            loss_dict = self.guidance(latent, lr_image, self.prompt_processor_output, c2w, mvp_mtx) #[NCHW]
            ...
            loss_vsd = loss_dict['loss_vsd']
            loss += loss_vsd
            self.log('train/loss_vsd', loss_vsd.item(), prog_bar=True)
